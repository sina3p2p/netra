...
Usage:
    # Single GPU
    python train.py --config configs/nano.yaml --data_path tokens.bin
...
from netra.data import MemmapTokenDataset
...
def parse_args():
    ...
    p.add_argument("--data_path", type=str, required=True,
                   help="Path to pre-tokenized tokens.bin")
...
def setup_device(ddp, local_rank):
    if ddp:
...
def build_model(args, tokenizer, device, rank, local_rank, world_size):
...
def build_dataloaders(args, model_config, rank, world_size, device):
    if not os.path.exists(args.data_path):
        raise FileNotFoundError(
            f"Data not found at {args.data_path}. "
            "Generate it first: python tools/tokenize_data.py"
        )
...
def buffer_eval_batches(eval_loader, n_batches):
    batches = []
...
def train_step(model, optimizer, scaler, train_iter, train_loader,
               args, device, ctx, ddp):
    """One full training step with gradient accumulation. Returns (loss, train_iter)."""
    ...
    for micro_step in range(args.grad_accum_steps):
        ...
        is_last = micro_step == args.grad_accum_steps - 1
        sync_ctx = nullcontext() if (not ddp or is_last) else model.no_sync()
...
def generate(model, tokenizer, prompt, max_tokens,
    ...
    cache = [{} for _ in range(len(raw.layers))]
    logits, _ = raw(ids, cache=cache)
    ...
    for _ in range(max_tokens):
        ...
        if next_id.item() == tokenizer.eot_id:
            break
        logits, _ = raw(next_id, cache=cache)
...
def main():
    ...
    if world_size > 1:
        ...
        if is_main:
            print(f"DDP: {world_size} GPUs → max_steps={args.max_steps}, "
                  f"warmup={args.warmup_steps}")

    device, dtype, ctx, scaler = setup_device(ddp, local_rank)
    ...
    model, model_config, start_step, ckpt = build_model(
        args, tokenizer, device, rank, local_rank, world_size,
    )
    ...
    train_loader, eval_loader = build_dataloaders(
        args, model_config, rank, world_size, device,
    )

    eval_batches = []
    if is_main:
        eval_batches = buffer_eval_batches(eval_loader, args.eval_steps)
        print(f"Buffered {len(eval_batches)} eval batches")

    # ── Wandb ─────────────────────────────────────────────────────────

    eff_batch_tokens = (args.batch_size * args.grad_accum_steps
                        * world_size * model_config.max_seq_len)

    if is_main:
        wandb.init(
            project="netra",
            name=args.wandb_run_name or f"netra-{config_name}",
    ...
    if is_main:
        print(f"\nEffective batch: {args.batch_size}×{args.grad_accum_steps}×{world_size} "
              f"= {args.batch_size * args.grad_accum_steps * world_size} seq "
              f"({eff_batch_tokens:,} tok)")
    ...
    for step in range(start_step, args.max_steps):
        ...
        step_loss, grad_norm, train_iter = train_step(
            model, optimizer, scaler, train_iter, train_loader,
            args, device, ctx, ddp,
        )
